...
- **Observed 2026-08-10:** the GitHub-Action-wrapped harness that fires these scheduled sessions assigns a **fresh per-run branch** (`claude/<slug>`) and its own instructions explicitly forbid pushing to `main` or any other branch without live-user permission, and forbid opening a PR unless asked — both of which are unavailable to a fully unsupervised run. This means "merge it into the default branch and push that" is **not achievable** by a scheduled session under the current harness config; the only reachable outcome is "push the branch and note it for the successor," and even the successor faces the same constraint on its own new branch. In practice a branch only reaches `main` when a human (or some other automation) merges its PR from outside the session — see `pipeline/state/runs/2026-08-10-1215.md` and the PR #1 merge in git history for the precedent. Until the owner wires up auto-merge or relaxes the harness restriction, **every scheduled run's work sits one manual merge away from `main`**, and deploy (stage g) cannot complete in the same run that built the app whenever this applies. Say so plainly in the run log rather than silently marking something "live" that isn't reachable yet.
